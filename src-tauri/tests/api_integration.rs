...
use chat_vault_lib::gemini_api::media_download::{DownloadStats, MediaDownloadItem};
use chat_vault_lib::gemini_api::GeminiExporter;
use chat_vault_lib::turn_parser::{
    normalize_turn_media_first_seen, parse_turn, MediaFile, ParsedTurn,
};
...
fn test_dr_conv_id() -> String {
    std::env::var("TEST_DR_CONV_ID")
        .expect("需设置环境变量 TEST_DR_CONV_ID（Deep Research 对话 ID）")
}
fn test_dr_conv_id_2() -> String {
    std::env::var("TEST_DR_CONV_ID_2")
        .expect("需设置环境变量 TEST_DR_CONV_ID_2（进行中的 Deep Research 对话 ID）")
}

async fn init_exporter() -> GeminiExporter {
    let all_cookies =
        cookies::get_cookies_from_local_browser().expect("无法从本机浏览器读取 cookies");
...
async fn dump_dr_conversation(
    ...
    for (i, turn) in raw_turns.iter().enumerate() {
        ...
        let _ = std::fs::write(
            &turn_file,
            serde_json::to_string_pretty(turn).unwrap_or_default(),
        );
        ...
        if let Some(md) = meta_dict {
            for key in &keys {
                ...
                if let Some(v) = md.get(key.as_str()) {
                    let _ =
                        std::fs::write(&kf, serde_json::to_string_pretty(v).unwrap_or_default());
                }
            }

            if let Some(data_58) = md.get("58") {
                let entry_count = data_58
                    .as_array()
                    .and_then(|a| a.get(1))
        ...
        if let Some(ref dr) = parsed.assistant.deep_research {
            let (dr_type, progress_len) = match dr {
                chat_vault_lib::turn_parser::DeepResearch::Plan { .. } => ("plan", 0),
                chat_vault_lib::turn_parser::DeepResearch::Report { progress, .. } => {
                    ("report", progress.as_ref().map(|p| p.len()).unwrap_or(0))
                }
            };
            eprintln!(
                "         parsed: type={}, progress_entries={}",
                dr_type, progress_len
            );
        } else {
...
async fn test_api_full_pipeline() {
    ...
    for (i, turn) in parsed_turns.iter().enumerate() {
        ...
        for f in &turn.user.files {
            eprintln!(
                "    [user]      type={:<12} file={:?}",
                f.media_type, f.filename
            );
        }
        for f in &turn.assistant.files {
            eprintln!(
                "    [assistant] type={:<12} file={:?}",
                f.media_type, f.filename
            );
        }
    ...
    eprintln!(
        "  filename={:?} mime={:?} duration={:?}",
        vid.filename, vid.mime, vid.duration
    );
    ...
    eprintln!(
        "  filename={:?} mime={:?} resolution={:?}",
        ai_img.filename, ai_img.mime, ai_img.resolution
    );
    ...
    eprintln!(
        "  model={:?} prompt={:?}",
        gm.model,
        gm.prompt.as_ref().map(|p| &p[..p.len().min(80)])
    );
    ...
    let music_turn_idx = parsed_turns
        .iter()
        .rposition(|t| t.assistant.files.iter().any(|f| f.media_type == "audio"));
    assert!(music_turn_idx.is_some(), "缺少 AI 生成音乐 turn");
    let music_turn = &parsed_turns[music_turn_idx.unwrap()];
    let music_audio = music_turn
        .assistant
        .files
        .iter()
    ...
    eprintln!(
        "  filename={:?} mime={:?}",
        music_audio.filename, music_audio.mime
    );
    ...
    let mm = music_meta_turn
        ...
        .unwrap();
    eprintln!(
        "  music_meta: title={:?} album={:?} genre={:?} moods={:?} caption={:?}",
        mm.title,
        mm.album,
        mm.genre,
        mm.moods,
    ...
    // type=unknown 文件
    let unknowns: Vec<(usize, &MediaFile)> = parsed_turns
        .iter()
        .enumerate()
        .flat_map(|(i, t)| {
            t.user
    ...
    let asst_att_filenames: Vec<Option<&str>> = parsed_turns
        .iter()
    ...
    for (i, t) in parsed2.iter().enumerate() {
        let txt = if t.user.text.len() > 50 {
            format!("{}...", &t.user.text[..50])
        } else {
            t.user.text.clone()
        };
        eprintln!("  turn[{}] {:?}", i, txt);
        ...
        if let Some(ref mm) = t.assistant.music_meta {
            eprintln!(
                "    music_meta: title={:?} album={:?} genre={:?} moods={:?}",
                mm.title, mm.album, mm.genre, mm.moods
            );
            eprintln!(
                "    caption: {:?}",
                mm.caption.as_ref().map(|c| &c[..c.len().min(100)])
            );
        }
        if let Some(ref gm) = t.assistant.gen_meta {
            eprintln!(
                "    gen_meta: model={:?} prompt={:?}",
                gm.model,
                gm.prompt.as_ref().map(|p| &p[..p.len().min(80)])
            );
        }
